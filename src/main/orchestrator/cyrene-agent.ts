...
export function toAguiEvent(event: AgentLoopEvent): BaseEvent {
  switch (event.type) {
    ...
    case "compressing_context":
      // 渲染端只把这当布尔触发信号用（AgentRunController 的 setCompressingContext），
      // 实际显示文案由 ChatWorkspaceNotices 通过 i18n key workspaceNotices.compressingContext
      // 自行渲染（随当前角色包动态换名）；这里不再携带会失配的硬编码文案。
      return { type: EventType.CUSTOM, name: "cyrene.compressingContext", value: {} };
    case "context_usage":
