...
// 注意：这里的 "昔涟" 特意保持字面量，不接入 getActiveCharacterDisplayName()。
// 本函数的输出会进入其他角色（万敌/丹恒等）的人设 prompt 正文——那是
// Moments 内部角色间的引用文本，不是直接展示给用户的 UI 标签；把它换成
// 当前角色包的展示名，会让"cyrene 内部角色"这一实现细节泄漏进和内容/时序
// 相关的模型决策链路，超出"只换用户可见的展示名"的既定范围。
