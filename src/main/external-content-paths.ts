...
import { DEFAULT_CHARACTER_PACK_ID } from "../shared/character-pack";
...
export interface ExternalContentPathInput {
  ...
  userDataPath: string;
  /** 当前生效的角色包 id；未传或等于 DEFAULT_CHARACTER_PACK_ID 时不叠加任何角色包目录。 */
  activeCharacterPackId?: string;
}
...
export interface ExternalContentPaths {
  ...
  /** 用户导入的角色包根目录（character-packs/<id>/...）。 */
 ...
 */
/**
 * 角色包会叠加在通用 prompt 目录之前（优先级最高，first-match-wins）：
 *  - 用户导入包：userData/character-packs/<id>/prompts
 *  - 内置附加包（如 generic）：installRoot/prompts/characters/<id>
 * 未指定或等于内置默认包（"cyrene"）时返回空数组，不改变既有行为。
 */
function characterPackPromptDirectories(
  packId: string | undefined,
  userCharacterPacksDir: string,
  installRoot: string,
): string[] {
  if (!packId || packId === DEFAULT_CHARACTER_PACK_ID) return [];
  const safeId = safeRelativePath(packId);
  if (!safeId || safeId.includes(path.sep)) return [];
  return [
    path.join(userCharacterPacksDir, safeId, "prompts"),
    path.join(installRoot, "prompts", "characters", safeId),
  ];
}

export function resolveExternalContentPaths(input: ExternalContentPathInput): ExternalContentPaths {
  if (!input.isPackaged) {
    const userCharacterPacksDir = path.join(input.userDataPath, "character-packs");
    return {
      installRoot: input.appPath,
      promptDirectories: [
        ...characterPackPromptDirectories(input.activeCharacterPackId, userCharacterPacksDir, input.appPath),
        path.join(input.appPath, "prompts"),
      ],
  ...
  return {
    installRoot,
    promptDirectories: [
      ...characterPackPromptDirectories(input.activeCharacterPackId, userCharacterPacksDir, installRoot),
...
/** 从磁盘直接读取当前生效的角色包 id，不经过 settings-facade 的完整规范化流程
 *  （避免 external-content-paths ↔ settings-facade 循环依赖）。读取失败一律回退为内置默认包。 */
function readActiveCharacterPackId(): string | undefined {
  try {
    const raw = JSON.parse(fs.readFileSync(getGeneralSettingsPath(), "utf8")) as {
      activeCharacterPackId?: unknown;
    };
    return typeof raw.activeCharacterPackId === "string" ? raw.activeCharacterPackId : undefined;
  } catch {
    return undefined;
  }
}
...
export function getExternalContentPaths(): ExternalContentPaths {
  try {
    return resolveExternalContentPaths({
      ...
      activeCharacterPackId: readActiveCharacterPackId(),
    });
  } catch {
