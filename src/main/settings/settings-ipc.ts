...
import {
  ...
  resolveActiveCharacterPack,
} from "../character-packs/character-pack-registry";
import { importCharacterPackZip } from "../character-packs/character-pack-import";
...
export function registerSettingsIpc(deps: SettingsIpcDependencies): void {
  ...
  ipc.handle(IPC.CHARACTER_PACKS_LIST, () => listInstalledCharacterPacks());
  ipc.handle(IPC.CHARACTER_PACKS_GET_ACTIVE, () => resolveActiveCharacterPack());

  ipc.handle(IPC.CHARACTER_PACKS_PICK_ZIP, async () => {
  ...
  ipc.handle(IPC.CHARACTER_PACKS_IMPORT_ZIP, async (_event, payload: unknown) => {
    const { zipPath, overwrite } = (payload ?? {}) as { zipPath?: unknown; overwrite?: unknown };
    if (typeof zipPath !== "string" || !zipPath) throw new Error("未选择角色包 zip 文件");
    return importCharacterPackZip({
      zipPath,
      userCharacterPacksDir: getExternalContentPaths().userCharacterPacksDir,
      overwrite: Boolean(overwrite),
    });
  });

  ipc.handle(IPC.CHARACTER_PACKS_DELETE, (_event, packId: unknown) => {
    if (typeof packId !== "string" || !packId) throw new Error("缺少角色包 id");
    return deleteCharacterPack(packId);
  });
