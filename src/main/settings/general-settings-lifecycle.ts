...
import { resolveActiveCharacterPack } from "../character-packs/character-pack-registry";
import { refreshTrayCharacterName } from "../tray";
...
export function handleGeneralSettingsChanged(
  ...
  if (before.activeCharacterPackId !== after.activeCharacterPackId) {
    deps.windowManager?.broadcast(IPC.CHARACTER_PACKS_CHANGED, resolveActiveCharacterPack());
    if (deps.tray) refreshTrayCharacterName(deps.tray);
  }
