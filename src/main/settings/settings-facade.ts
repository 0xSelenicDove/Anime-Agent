...
export function normalizeGeneralSettings(
  ...
  return {
    // 仅做格式校验（id 合法性），不在此处检查角色包是否真实存在——
    // 那属于 character-pack-registry 的解析/回退职责，避免 settings-facade
    // 与 character-pack-registry 产生循环依赖。
    activeCharacterPackId: isValidCharacterPackId(input?.activeCharacterPackId)
      ? input.activeCharacterPackId
