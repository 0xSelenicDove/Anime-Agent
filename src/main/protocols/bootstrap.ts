...
import { parseCharacterPackUrl, resolveCharacterPackAssetPath } from "../character-packs/character-pack-protocol";
...
export function registerPrivilegedSchemes(): void {
  protocol.registerSchemesAsPrivileged([
    ...
    { scheme: "character-pack", privileges: { standard: true, secure: true, supportFetchAPI: true, stream: true } },
  ]);
}
...
export function registerProtocolHandlers(): void {
  ...
  protocol.handle("character-pack", (request) => {
    const parsed = parseCharacterPackUrl(request.url);
    if (!parsed) return new Response("Invalid character pack URL", { status: 404 });

    const { userCharacterPacksDir } = getExternalContentPaths();
    const filePath = resolveCharacterPackAssetPath(userCharacterPacksDir, parsed.packId, parsed.file);
    if (!filePath || !fs.existsSync(filePath)) return new Response("Character pack asset not found", { status: 404 });
