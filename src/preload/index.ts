...
const characterPacksApi = {
  list: () => ipcRenderer.invoke(IPC.CHARACTER_PACKS_LIST),
  getActive: () => ipcRenderer.invoke(IPC.CHARACTER_PACKS_GET_ACTIVE),
  onChanged: (callback: (pack: unknown) => void) => {
    const listener = (_e: unknown, pack: unknown) => callback(pack);
    ipcRenderer.on(IPC.CHARACTER_PACKS_CHANGED, listener);
    return () => ipcRenderer.off(IPC.CHARACTER_PACKS_CHANGED, listener);
  },
  pickZip: () => ipcRenderer.invoke(IPC.CHARACTER_PACKS_PICK_ZIP) as Promise<string | null>,
  importZip: (zipPath: string, overwrite = false) =>
    ipcRenderer.invoke(IPC.CHARACTER_PACKS_IMPORT_ZIP, { zipPath, overwrite }),
  delete: (packId: string) => ipcRenderer.invoke(IPC.CHARACTER_PACKS_DELETE, packId),
};

contextBridge.exposeInMainWorld("characterPacks", characterPacksApi);
