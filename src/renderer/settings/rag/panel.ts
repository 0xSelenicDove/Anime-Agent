      ...
      try {
        ...
        if (result?.ok) {
          ...
          if (result.clearedEntries && result.clearedEntries > 0) {
            window.alert(t("rag-panel.1") + result.clearedEntries + t("rag-panel.2"));
          }
        } else {
          // Rollback on failure
          cards.forEach((c) => c.classList.remove("is-active"));
          ...
          window.alert(t("rag-panel.3") + (result?.error || t("rag-panel.4")));
        }
      } catch (err) {
        // Rollback on error
        cards.forEach((c) => c.classList.remove("is-active"));
...
(async () => {
  ...
  try {
    ...
    if (!status) return;
    if (standardEl) standardEl.textContent = status.standard ? t("rag-panel.5") : t("rag-panel.6");
  } catch (err) {
    console.warn("[Reranker] status check failed:", err);
    if (standardEl) standardEl.textContent = t("rag-panel.7");
  }
})();
...
(async () => {
  ...
  try {
    ...
    if (!status) {
      if (bgem3El) bgem3El.textContent = t("rag-panel.8");
      return;
    }
    if (bgem3El) bgem3El.textContent = status.embedding?.bgem3 ? t("rag-panel.9") : t("rag-panel.10");
  } catch (err) {
    console.warn("[Embedding] status check failed:", err);
    if (bgem3El) bgem3El.textContent = t("rag-panel.11");
  }
})();
...
(function () {
  ...
  function _showModal(opts: { title: string; message: string; icon?: string; confirmText?: string; cancelText?: string }): Promise<boolean> {
    ...
    if (!ov) {
      ...
      ov.innerHTML = `<div class="cy-modal" role="alertdialog" aria-modal="true"><div class="cy-modal__head"><span class="cy-modal__icon" id="cy-modal-icon">📌</span><h3 class="cy-modal__title" id="cy-modal-title">${t("rag-panel.12")}</h3></div><hr class="cy-modal__divider"><p class="cy-modal__body" id="cy-modal-message">${t("rag-panel.13")}</p><div class="cy-modal__actions"><button type="button" class="ghost-btn" id="cy-modal-cancel">${t("rag-panel.14")}</button><button type="button" class="btn-primary" id="cy-modal-confirm">${t("rag-panel.15")}</button></div></div>`;
    ...
    cancelBtn.textContent = opts.cancelText || t("rag-panel.16");
    confirmBtn.textContent = opts.confirmText || t("rag-panel.17");
  ...
  deleteBtn?.addEventListener("click", async () => {
    ...
    const name = "BGE-M3";
    var confirmed = await _showModal({ title: t("rag-panel.18"), message: t("rag-panel.19") + name + t("rag-panel.20"), icon: "⚠️", confirmText: t("rag-panel.21"), cancelText: t("rag-panel.22") });
    if (!confirmed) return;
    deleteBtn.disabled = true;
    deleteBtn.textContent = t("rag-panel.23");
    try {
      const result = await window.settings?.deleteEmbeddingModel?.(model);
      if (result?.ok) {
        deleteBtn.textContent = t("rag-panel.24");
        setTimeout(() => location.reload(), 800);
      } else {
        deleteBtn.textContent = t("rag-panel.25");
        deleteBtn.disabled = false;
      }
    } catch (err) {
      deleteBtn.textContent = t("rag-panel.26");
      deleteBtn.disabled = false;
    }
  });
...
(function () {
  ...
  updateBtn?.addEventListener("click", () => {
    updateBtn.textContent = t("rag-panel.27");
    updateBtn.disabled = true;
    setTimeout(() => {
      updateBtn.textContent = t("rag-panel.28");
