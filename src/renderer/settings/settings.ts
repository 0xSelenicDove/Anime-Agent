...
import "./character/panel";  // 副作用导入：角色包列表加载 + 切换
...
import { applySettingsLanguage, onSettingsLanguageApplied, subscribeSettingsLanguageChanges, t } from "./i18n";

subscribeSettingsLanguageChanges();
onSettingsLanguageApplied(() => switchSection(lastSection));
...
const NAV_LABEL_ICONS: Record<string, string> = {
  ...
  "api-advanced": `<svg width="24" height="24" viewBox="0 0 48 48" fill="none" aria-hidden="true" style="vertical-align:-3px"><title>高级设置</title><path d="M34.0003 41L44 24L34.0003 7H14.0002L4 24L14.0002 41H34.0003Z" fill="none" stroke="currentColor" stroke-width="4" stroke-linejoin="round"/><path d="M24 29C26.7614 29 29 26.7614 29 24C29 21.2386 26.7614 19 24 19C21.2386 19 19 21.2386 19 24C19 26.7614 21.2386 29 24 29Z" fill="none" stroke="currentColor" stroke-width="4" stroke-linejoin="round"/></svg>`,
  character: `<svg width="24" height="24" viewBox="0 0 48 48" fill="none" aria-hidden="true" style="vertical-align:-3px"><title>角色</title><path d="M6 20C6 12 13 6 24 6C35 6 42 12 42 20C42 25 37 27 31 24L24 30L17 24C11 27 6 25 6 20Z" stroke="currentColor" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/><circle cx="16" cy="17" r="3" stroke="currentColor" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/><circle cx="32" cy="17" r="3" stroke="currentColor" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/></svg>`,
  cyrene: "🌸",
  tts: "🎙️",
  asr: "🎧",
...
function switchSection(section: string): void {
  ...
  if (characterPanel) characterPanel.classList.toggle("is-hidden", !isCharacter);
  ...
  placeholderPanel.classList.toggle(
    "is-hidden",
    isApi || isApiAdvanced || isAppearance || isGeneral || isPreferences || isCyrene || isCharacter || isDisclaimer || isMemory || isUser || isTasks || isPlugins || isTokens || isChannels || isTts || isAsr || isMusic,
  );
