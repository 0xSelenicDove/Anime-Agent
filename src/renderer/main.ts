...
import { getActiveCharacterPack, resolveCharacterPackAssetUrl } from "./shared/apply-active-character";
...
const DEFAULT_MODEL_PATH = resolveAsset("models/cyrene/Cyrene.model3.json");
const DEFAULT_MODEL_SIGNATURE = "default";

/** 解析当前生效角色包应使用的 Live2D 模型；未声明自定义模型时回退内置默认模型。
 *  signature 用于判断角色包切换后模型是否真的变了（避免只换了名字/头像也重载窗口）。 */
async function resolveActiveModel(): Promise<{ path: string; signature: string }> {
  const pack = await getActiveCharacterPack();
  if (pack?.manifest.hasCustomModel && pack.manifest.modelEntryFile) {
    return {
      path: resolveCharacterPackAssetUrl(pack, pack.manifest.modelEntryFile),
      signature: `${pack.manifest.id}:${pack.manifest.modelEntryFile}`,
    };
  }
  return { path: DEFAULT_MODEL_PATH, signature: DEFAULT_MODEL_SIGNATURE };
}

let manager: Live2DManager | undefined;
let loadedModelSignature = DEFAULT_MODEL_SIGNATURE;

async function bootLive2D(): Promise<void> {
  const { path: modelPath, signature } = await resolveActiveModel();
  loadedModelSignature = signature;
  const instance = new Live2DManager({
    canvas,
    ...
    speechOffs.push(
      trackSubscription("live2dAction:onPlayAction", window.live2dAction?.onPlayAction((target) => {
        void instance.playAction(target);
      }) ?? (() => {})),
    );
    live2dSpeechOffs = speechOffs;
    interaction = new InteractionController(canvas, model, instance.getHitAreaDefs(), {
      onTrigger: (area) => {
    ...
    clickThrough = new ClickThroughController(canvas, instance, {
    ...
    // the model to match.
    petZoomOff = trackSubscription("cyrene:onPetZoom", window.cyrene.onPetZoom((zoom) => instance.applyZoom(zoom)));
    ...
    window.settings?.getGeneral().then((cfg) => {
      if (cfg?.petZoom && cfg.petZoom !== 1) {
        instance.applyZoom(cfg.petZoom);
      }
    }).catch(() => { /* 设置读取失败不影响加载 */ });
    ...
    (window as unknown as { __cyrene: unknown }).__cyrene = {
      ...
      getLive2DDiagnostics: () => ({
        resources: instance.getResourceMetrics(),
...
// 角色包切换后，只有当解析出的模型 signature 真的变了（换了一个带自定义模型的包，
// 或从自定义模型换回内置默认）才重载窗口；纯改名字/头像不触发，避免不必要的闪烁。
window.characterPacks?.onChanged(() => {
  void resolveActiveModel().then(({ signature }) => {
    if (signature !== loadedModelSignature) location.reload();
  });
});
