...
import { useActiveCharacterAvatarUrl, useActiveCharacterPack } from "../../../hooks/useActiveCharacterPack";
...
export function ChatComposer({
  ...
  const { t } = useTranslation();
  const activeCharacterPack = useActiveCharacterPack();
  const activeCharacterAvatarUrl = useActiveCharacterAvatarUrl();
  // 四张欢迎插画是昔涟专属手绘立绘（角色形象本身画进了图里），不是"文字换个名字"能通用的东西；
  // 非默认角色包（含用户导入的自定义包）一律退化成头像 + 文字的轻量版欢迎语，不借用这套画风。
  const isDefaultCharacterPack = activeCharacterPack.manifest.id === DEFAULT_CHARACTER_PACK_ID;
  ...
  return (
    <div className={`cy-composer-stack ${docked ? "is-docked" : "is-centered"}`}>
      {!docked && (
        isDefaultCharacterPack ? (
          <img className="cy-composer-welcome" src={welcomeImageUrl} alt="" />
        ) : (
          <div className="cy-composer-welcome-fallback">
            <img className="cy-composer-welcome-fallback__avatar" src={activeCharacterAvatarUrl} alt="" />
            <span className="cy-composer-welcome-fallback__text">{placeholder}</span>
          </div>
        )
      )}
